...
file_put_contents("tmp.html",$matriz[398],FILE_APPEND);
...
foreach ( $links as $link )
{
   ...
   $mat = file($clink);
   //escribimos el enlace corregido mas la descripcion concreta de la patente (se encuentra en la linea 402 del html generado)
   $clink = '<p>'.'<a href='.$clink.'title="Enlace" target="new">Enlace original</a>'.'</p>';
   $salida = $salida.$clink;
   $salida = $salida.utf8_encode($mat[402]);
}
//reparamos las url relativas
$url_repair='<a href="/es/signos_distintivos/[^>]*resultado';
$salida = eregi_replace($url_repair,'<a href="http://www.oepm.es/es/signos_distintivos/resultado',$salida);
...
$limpiar_checkbox='<input type="checkbox"[^>]*>';
$url90 = eregi_replace($limpiar_checkbox,'',$url90);
//areglamos los enlaces relativos referidos a la web original
$url_repair='<a  href="/publicationDetails/[^>]*biblio';
$url90 = eregi_replace($url_repair,'<a  href="http://worldwide.espacenet.com/publicationDetails/biblio',$url90);
...
echo '<p>'.'Es necesario habilitar las ventanas emergentes para poder visualizar los resultados de la búsqueda'.'</p>';
